...
class EML_Processing:
    ...
    def save_results(self, dirname, metadata_average_s=None):
        ...
        if self.despiked:
            self.core_loss_despiked.save(
                os.path.join(dirname, "core_loss_despiked.hspy"), overwrite=True
            )
        ...
        if hasattr(self, "mask_vacuum"):
            ...
            for i in self.deconvolved_edge:
                ...
                self.average_spectrum_excluding_vacuum_signal.metadata.Analysis = str(
                    metadata_average_s
                )
    ...
    def zlp_align(self):
        ...
        if (
            self.low_loss.data.shape[-1] % 2 == 1
        ):  # pyEELSMODEL has problem with spectra with odd number of channels ((internal anger))
    ...
    def despike(self, si, threshold="auto"):
        ...
        if threshold == "auto":
            ...
            fomd = np.nan_to_num(fomd, nan=0.0, posinf=0.0, neginf=0.0)
            his, bins = np.histogram(fomd.ravel(), bins=self.n_bins)
            j = (
                si.estimate_elbow_position(
                    explained_variance_ratio=his * 100 / his.sum(), max_points=len(his)
                )
                - 1
            )
    ...
    def create_denoised(self):
        ...
        if self.despiked:
            ...
            self.denoised = self.core_loss_despiked.get_decomposition_model(
                self.core_loss_despiked.estimate_elbow_position()
            )

        else:
            self.core_loss.decomposition()
            self.denoised = self.core_loss.decomposition(
                self.core_loss.estimate_elbow_position()
            )
    ...
    def create_pyeels_fit(self, use="denoised"):
        ...
        hl = em.MultiSpectrum.from_numpy(
            data, self.core_loss_despiked.axes_manager[-1].axis
        )
        ll = em.MultiSpectrum.from_numpy(
            self.low_loss.data, self.low_loss.axes_manager[-1].axis
        )
        ...
        self.llcomp = MscatterFFT(
            ll.get_spectrumshape(),
            ll,
            True,
            self.padding_mode_data,
            self.padding_mode_llspectrum,
        )
        ...
        bg = LinearBG(
            specshape=hl.get_spectrumshape(),
        ...
        for f in self.edges_with_fine_structure:
            ...
            corresponding = [
                i
                for i in self.comp_elements
                if (i.element == element) and i.edge == edge
            ][0]
            ...
            if f in self.edges_with_fine_structure_parameters:
                ...
                interpolationtype = self.edges_with_fine_structure_parameters[f][
                    "interpolationtype"
                ]
            else:
                pre_e = self.pre_e
    ...
    def remove_plural_scattering_via_model_fitting(self, element, edges):
        ...
        model = [
            i
            for i in self.fit.model.components
            if (check_element(i) == element and check_edge(i) in edges)
        ]
        ...
        if not hasattr(self, "llcomp_zlp"):
            ...
            ll_zlp = em.MultiSpectrum.from_numpy(
                zlp, self.low_loss.axes_manager[-1].axis
            )
            llcomp_zlp = MscatterFFT(
                ll_zlp.get_spectrumshape(),
                ll_zlp,
                True,
                self.padding_mode_data,
                self.padding_mode_llspectrum,
            ...
            self.fit.processing_llcomps = {
                "llcomp": self.llcomp,
                "llcomp_zlp": self.llcomp_zlp,
            }
...
def kill_channel_range(signal, coord_range):
    ...
    if ei == ef:
        ...
        I_intp = I_intp_i + ((I_intp_f - I_intp_i) / (e_intp_f - e_intp_i)) * (
            x_intp - x_intp[0]
        )
