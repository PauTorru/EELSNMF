...
def Factor(th, alpha, beta):
    ...
    for i, t in enumerate(th):
        if t <= abs(alpha - beta):
            ...
            sqrt = np.sqrt(
                4 * (alpha**2) * (beta**2) + (alpha**2 + beta**2 - t**2) ** 2
            )
            f = (1 / np.pi) * (
                np.arccos(x)
...
def integral_over_th(thE, alpha, beta, n_points):
    ...
    integrand = (Fth * 2 * np.pi * th * dth)[:, np.newaxis] / (
        th[:, np.newaxis] ** 2 + thE[np.newaxis, :] ** 2
    )
...
def adapt_G_SumRule(self, convergent_probe_correction=False, n_points=1000):
    ...
    sigma = np.array(
        [self.em_fitter.A_matrix[:, v] for k, v in self.xsection_idx.items()]
    )
    list_emax = [
        self.ax.value2index(v[1]) for k, v in self.fine_structure_ranges.items()
    ]
    Gs = sigma.copy()
    ...
    if convergent_probe_correction:
        pp = convergent_psi(
            self.energy_axis, self.alpha, self.beta, self.E0, n_points=n_points
        )
    else:
...
def init_XWH_sumrules(self):
    ...
    # 	else:
    GW, self.sum_rules.H = initialize_nmf(
        self.X, self.n_comps, init=self.init_nmf, random_state=self.random_state_nmf
    )
    self.sum_rules.W = np.abs(
        np.linalg.lstsq(
            self.sum_rules.Gf
            + self.sum_rules.Gs @ self.sum_rules.K @ self.sum_rules.Bf,
            GW,
            rcond=None,
        )[0]
    )
