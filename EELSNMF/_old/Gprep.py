...
def build_G(self):
    ...
    if len(self.cl.data.shape) == 2:
        ...
        hl = em.MultiSpectrum.from_numpy(
            self.cl.data.reshape((1, p, e)), self.cl.axes_manager[-1].axis
        )
    else:
    ...
    if self.background_exps is not None:
        bg = LinearBG(hl.get_spectrumshape(), rlist=self.background_exps)
    else:
        bg = LinearBG(
            hl.get_spectrumshape(),
    ...
    for i, k in enumerate(self.fine_structure_ranges.keys()):
        self._edge_slices[k] = np.s_[
            :, cs[i] : cs[i + 1]
        ]  # this gives you the slice to get a given edge: G[slice]@W[slice,comp] for and edge of a component
    ...
    for k in self.xsection_idx.keys():
        ...
        self.G[ii : ff + 1, self.xsection_idx[k]] = (
            0  # ff+2 avoids overlap with fine structure?
        )
...
def _prepare_single_spectrum_convolution_G(self, G):
    ...
    if self.llspectrum.shape[-1] > G.shape[0]:
        ...
        self.llspectrum = np.pad(
            self.llspectrum, (0, missing), mode="constant", constant_values=(0, 0)
        )
    else:
    ...
    for k, v in self.fine_structure_ranges.items():
        ...
        ii, ff = self.ax.value2index(v)
        l = ff - ii + 1  # this overlaps with cropped xsection. problematic?
        G[:ff, self.n_background + ne :] = (
            0  # ne is to only set to zero the corresponding edge #[ii:ff,self.n_background+ne:]=0
        )
...
def _prepare_full_convolution_G(self, G0):
    ...
    for p in range(
        ...
        if not hasattr(self, "GtG"):  # init here to no have to count final G size
            self.GtG = np.zeros(
                [G.shape[1], G.shape[1], self.X.shape[1]], dtype=self.dtype
            )
