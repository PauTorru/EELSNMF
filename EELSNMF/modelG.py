    ...
    def __init__(self, parent, low_loss_spectrum):
        ...
        for k, v in self.parent.fine_structure_ranges.items():
            ...
            for r, i in enumerate(range(ii, ff + 1)):
                ...
                if i > o:
                    freeG[i - o :, r] = (
                        self.llspectrum_data[: -(i - o)]
                        / self.llspectrum_data[: -(i - o)].sum()
                    )
                elif i < o:
                    freeG[: -(o - i), r] = (
                        self.llspectrum_data[o - i :]
                        / self.llspectrum_data[o - i :].sum()
                    )
...
MODEL_REGISTRY = {"deltas": Deltas, "convolved_single": ConvolvedSingle}
