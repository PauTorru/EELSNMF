...
class ListOfSI:
    ...
    def __init__(self, slist, energy_axis=None):
        ...
        unfolded_data = np.zeros(
            ((self.dim_list[:, 0] * self.dim_list[:, 1]).sum(), self.dim_list[0, -1])
        )
        ...
        for s in self.slist:
            ...
            unfolded_data[filled : filled + sx * sy, :] = s.data.reshape(
                (-1, self.edim)
            )
            filled += sx * sy
    ...
    def fold_array(self, array):
        ...
        for n, d in zip(self.spatial_dim_list, self.dim_list):
            folded_array = array[filled : filled + n, ...].reshape(
                tuple(d[:-1]) + (-1,)
            )
    ...
    def plot_cluster_results(self):
        ...
        self.plot_array(l, False, cmap=cmap, vmin=0.0, vmax=n - 1)
        r, c = self.plot_structure
        ax = plt.subplot(r, c, r * c)
        for i in range(f.data.shape[0]):
            plt.plot(
                self.energy_axis,
                f.data[i],
                color=sns.color_palette(cc.glasbey, n).as_hex()[i],
            )
