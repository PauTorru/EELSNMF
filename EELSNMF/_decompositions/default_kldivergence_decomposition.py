...
class Default_KL:
    ...
    def KL_divergence_error(self):
        if hasattr(
            self, "GW"
        ):  # not the actual error (H is updated after computing self.GW) but faster to compute
            return (
                self.X
                * self.xp.log((self.X + self.eps) / (self.GW @ self.H + self.eps))
                - self.X
                + self.GW @ self.H
            ).sum()
        else:
            return (
                self.X
                * self.xp.log(
                    (self.X + self.eps) / (self.G @ self.W @ self.H + self.eps)
                )
                - self.X
                + self.G @ self.W @ self.H
            ).sum()
