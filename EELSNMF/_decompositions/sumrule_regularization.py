...
class LogSumRule_Regularization:
    ...
    def _calcB(self, edge):
        ...
        if self.model._G_rescaled_to1:
            G0 = (
                self._G0[:, self.model.xsection_idx[edge]]
                / self.model._scales[self.model.xsection_idx[edge]]
            )
        else:
    ...
    def _LogSumRule_penalty(self):
        ...
        for i, edge in enumerate(self.edges):
            x = (
                (
                    getattr(self, self._edge_psi[edge])[:, None]
                    * self.W[self._edge_indices[edge], :]
                ).sum(0)
                + self.delta
            ) / (
                getattr(self, self.B[edge]) * self.W[self.model.xsection_idx[edge], :]
                + self.delta
            )
    ...
    def _LogSumRule_gradient(self):
        ...
        for edge in self.edges:
            ...
            elnes_term = (
                getattr(self, self._edge_psi[edge])[:, None] * self.W[v, :]
            ).sum(0)
            ...
            self._dJcdW[v, :] = (
                log_ratio
                * getattr(self, self._edge_psi[edge])[:, None]
                / (elnes_term[None, :] + self.delta)
            )

            # xs coeficients
            self._dJcdW[i, :] = (
                -log_ratio * getattr(self, self.B[edge]) / (xs_term + self.delta)
            )

            # for edges below tolerance set gradient to 0:
            abs_lr = self.xp.abs(log_ratio)
            exceed_tolerance = abs_lr >= self.SR_tolerance
    ...
    def SumRule_decomposition(
        ...
        SR_tolerance: float
                ...
                The tolarence is applied on ELNES/XSection ratio, so SR_tolerance = 1 means no tolerance.
