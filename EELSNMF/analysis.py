...
class Analysis:
    ...
    def _default_component_quantification_method(self):
        ...
        qm = default_q_methods[
            (
                self.analysis_description["model_type"],
                self.analysis_description["decomposition"]["method"],
            )
    ...
    def get_chemical_maps(self, quantified=True, method=None):
        ...
        if quantified:
            array_spatial_quantification *= (
                100 / array_spatial_quantification.sum(0)[np.newaxis, ...]
            )
    ...
    def evaluate_comp_contributions(self, edge=None):
        ...
        for k in range(self.n_components):
            ...
            evk = (
                2 * interaction_X - comp_sq
            ) / normX  # expansion of (||X||-||X-Comp||)/||X||
            ev.append(evk)

            interaction_Emodel = wk.T @ Emodel @ hk.T
            Ck = (
                (comp_sq + 2 * interaction_Emodel) / normX
            )  # expansion of ||Error_withoutK||-||Error_fullmodel||=||X-(Model-Comp)||-||X-Model||
            # is equal to power if the model is fully converged
            loo.append(Ck)

        loo, power, ev = [np.array(i) for i in [loo, power, ev]]
        print(f"Total explained : {R2}")
        fancy_summary = pd.DataFrame(
            data={
